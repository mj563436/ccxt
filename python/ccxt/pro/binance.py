...
class binance(ccxt.async_support.binance):
    ...
    def handle_ohlcv(self, client: Client, message):
        ...
        unicorn_fied_data = {
            'stream_type': messageHash,
            'event_type': event,
            'event_time': self.safe_value(message, 'E'),
            'symbol': marketId,
            'kline': {
                'kline_start_time': self.safe_value(kline, 't'),
                'kline_close_time': self.safe_value(kline, 'T'),
                'symbol': self.safe_value(kline, 's'),
                'interval': self.safe_value(kline, 'i'),
                'first_trade_id': self.safe_value(kline, 'f'),
                'last_trade_id': self.safe_value(kline, 'L'),
                'open_price': self.safe_value(kline, 'o'),
                'close_price': self.safe_value(kline, 'c'),
                'high_price': self.safe_value(kline, 'h'),
                'low_price': self.safe_value(kline, 'l'),
                'base_volume': self.safe_value(kline, 'v'),
                'number_of_trades': self.safe_value(kline, 'n'),
                'is_closed': self.safe_value(kline, 'x'),
                'quote': self.safe_value(kline, 'q'),
                'taker_by_base_asset_volume': self.safe_value(kline, 'V'),
                'taker_by_quote_asset_volume': self.safe_value(kline, 'Q'),
                'ignore': self.safe_value(kline, 'B'),
            },
        }
        ...
        marketType = 'spot' if (isSpot) else 'contract'
        symbol = self.safe_symbol(marketId, None, None, marketType)
        self.ohlcvs[symbol] = self.safe_value(self.ohlcvs, symbol, {}) # create caching dict
        stored = self.safe_value(self.ohlcvs[symbol], timeframe)
        # create an empty list inside ohlvcs dict.
        # {'AMB/USDT:USDT': {'1m': []}}
        if stored is None:
